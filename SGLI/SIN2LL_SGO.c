...
/* Sinusoidal to LAT/LNG with Gain offset corr (short) */
...
int main(int argc ,char *argv[])
{
    ...
    if(argc!=2)
    {
        fprintf(stderr,"Usage: SIN2LL_SGO paramfile\n");
    ...
    if((SINIMG=(short*)malloc(sizeof(short)*LRECL))==NULL)
    {
    ...
    for(i=0;i<NL;i++)
    {
        fread(SINIMG,sizeof(short),LRECL,f);
	...
	for(j=0;j<LLRECL;j++)
	{
	    ...
	    if(SINIMG[SATX]==XXXNUL)
	        ...
	        LLIMG[j]=G*(double)SINIMG[SATX]+O;
	}
